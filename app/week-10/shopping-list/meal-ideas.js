              ...
              {!loading && !error && meals.length > 0 && (
                <div className="flex">
                    ...
                    {meals.map((meal) => (
                      <li
                        ...
                        className={`bg-dark-slate p-4 rounded-md shadow-md mb-4 border-white border hover:scale-105 transition-transform duration-300 cursor-pointer ${
                          meal === selectMeal ? "selected bg-gray-400" : ""
                        }`}
                        onClick={() => {
                          if (meal === selectMeal) {
                            handleNotSelectMeal();
                  ...
                  <div className="flex-grow flex flex-col">
                    {selectMeal && (
                      <div className={`bg-dark-slate p-4 rounded-md shadow-md mb-4 border-white border hover:scale-105 transition-transform duration-300 ${selectMeal ? "selected bg-gray-400" : ""}`}>
...
async function getMealIdeas(ingredient) {
    try {
        ...
        if (data.meals && data.meals.length > 0) {
            const MealDetailPromise = data.meals.map((meal) => fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${meal.idMeal}`) 
            .then((res) => res.json()));
